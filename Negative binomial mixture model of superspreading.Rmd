...
ggsave("Figure2.pdf", pmfplot, height=9, width=14.5, units="cm")
pmfplot
```
...
#   geom_point(aes(x = Chains, y = Proportion, shape=Model, colour=Model), alpha=1)+
#   geom_line(aes(x = Chains, y = Proportion, colour=Model), 
#             linetype="dotted", alpha=.6, show.legend = FALSE)+
...
ggsave("Figure3.pdf", pmfplot2, height=9, width=14.5, units="cm")
pmfplot2
...
ggsave("Figure4.pdf", CompCumuDistplot, height=9, width=14.5, units="cm")
CompCumuDistplot
...
mstatdata<-data.frame(model="mixture", R0=rep(R0, length(p)), R0D=rep(R0D , length(p)),p=p,delta=delta, k=rep(k[1], length(p)), CaseVariance=mixstat[1,,1], ExtinctionProbability=mixstat[1,,2],R0star=mixstat[1,,3],ChainVariance=mixstat[1,,4], ProbabilityChainSizeLessEqual10=mixstat[1,,5], ProbChainSize1=mixstat[1,,6])
...
probzerosecondplot<-
ggplot(statdata)+
   geom_point(aes(x = p, y = ProbChainSize1, colour=model))+
  geom_line(aes(x = p, y = ProbChainSize1, colour=model))+
  facet_wrap(~k, labeller=label_both)+
  ylab("Probability of zero secondary infections")+xlab("Proportion superspreaders p")
...
ggsave("majoroutbreakplot.pdf", majoroutbreakplot, height=4, width=5)
majoroutbreakplot
```
...
ggsave("minoroutbreakplot.pdf", minoroutbreakplot, height=4, width=5)
majoroutbreakplot
```
...
ggsave("meanchainplot.pdf", meanchainplot, height=4, width=5)
meanchainplot
```
...
ggsave("CVchainplot.pdf", CVchainplot, height=4, width=5)
CVchainplot
```
...
ggsave("Figure5.pdf", controlplot, height=13, width=14.5, units="cm")
```
