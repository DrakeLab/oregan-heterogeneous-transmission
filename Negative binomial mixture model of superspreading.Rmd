...
NegBinomMixProbZero<-function(p, R0D, k, delta){
  R0A<-R0D + delta
  prob<-(p)*(1+R0A/k)^(-k)+(1-p)*(1+R0D/k)^(-k)
}

NegBinomProbZero<-function(R0, k){
  prob<-(1+R0/k)^(-k)
}
...
bdatapmf<-data.frame(Cases=0:10, Proportion=basedata[,1], k=rep(k[1],11), Model=rep("standard",11),R0=rep(2,nend+1), Distribution = rep("pmf",nend+1))
for(i in 2:4) bdatapmf<-bind_rows(bdatapmf,data.frame(Cases=0:10, Proportion=basedata[,i], k=rep(k[i],11), Model=rep("standard",11),R0=rep(2,nend+1), Distribution = rep("pmf",nend+1)))
...
pmfplot<-
ggplot(pmf)+
  geom_bar(aes(x = Cases, y = Proportion, fill = Model), 
           position = "dodge", stat = "identity", width=.3)+
  scale_x_continuous(breaks=0:10)+
  facet_wrap(~k, labeller=label_bquote(k == .(k)))+
  xlab("Number of secondary infections N = j")+ylab("Probability P(N=j)")+
  theme_minimal()+
  theme(panel.grid.major.x = element_blank(),
        panel.grid.minor=element_blank(), 
        panel.border = element_rect(color = "darkgrey", fill = NA, size = 1))+
  scale_fill_manual(values=c("#000000","#5798d1"))

ggsave("pmfplot.pdf", pmfplot, height=9, width=14.5, units="cm")
pmfplot
```
...
clusterbase<-data.frame(Chains=1:nend, Proportion=csizebase[,1], CumuProb= cumsum(csizebase[,1]), CompCumuProb=1-cumsum(csizebase[,1]), MajorOutbreak= rep(1-uniroot(NegBinomGen, c(0, 0.99),   R0=R0, k=k[1])$root, nend),k=rep(k[1],nend), Model=rep("standard",nend), R0=rep(2,nend), Distribution = rep("chain",nend)) 
for(i in 2:4) clusterbase<-bind_rows(clusterbase,data.frame(Chains=1:nend, Proportion=csizebase[,i], CumuProb= cumsum(csizebase[,i]), CompCumuProb=1-cumsum(csizebase[,i]),MajorOutbreak= rep(1-uniroot(NegBinomGen, c(0, 0.99),   R0=R0, k=k[i])$root, nend),k=rep(k[i],nend), Model=rep("standard",nend), R0=rep(2,nend), Distribution = rep("chain",nend)))
...
ggplot(clusterdistrib)+
  geom_bar(aes(x = Chains, y = Proportion, fill = Model), 
           position = "dodge", stat = "identity", width=.3)+
  scale_x_continuous(breaks=1:10, limits = c(0,10))+
  facet_wrap(~k, labeller=label_bquote(k == .(k)))+
  xlab("Chain size Y=y") + ylab("Probability P(Y=y)")+
  theme_minimal()+
  theme(panel.grid.major.x = element_blank(),
        panel.grid.minor=element_blank(), 
        panel.border = element_rect(color = "darkgrey", fill = NA, size = 1))+
...
ggsave("pgfplot.pdf", pgfplot, height=9, width=14.5, units="cm")
pgfplot
...
probmajoroutbreak <- clusterdistrib %>% 
  group_by(k,Model) %>% 
  summarise(ProbMajorOutbreak = max(MajorOutbreak))

CompCumuDistplot<-
ggplot(clusterdistrib)+
  geom_bar(aes(x = Chains, y = CompCumuProb, fill = Model), 
           position = "dodge", stat = "identity", width=.5)+
  geom_segment(aes(y = ProbMajorOutbreak, yend = ProbMajorOutbreak, colour = Model),
               data=probmajoroutbreak,
               x = 0, xend = 21, size=.3)+
  geom_text(aes(y = ProbMajorOutbreak, colour = Model, label=round(ProbMajorOutbreak,3)), 
            data = probmajoroutbreak, 
            x = 21, size = 3, hjust = 0)+
  scale_x_continuous(breaks=seq(0, 20, 5), 
                     ...
                     limits = c(0,23))+
  facet_wrap(~k, labeller=label_bquote(k == .(k)))+
  xlab("Chain size y") + ylab("Proportion of chains > y")+
  theme_minimal()+
  theme(panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank(), 
        panel.border = element_rect(color = "darkgrey", fill = NA, size = 1))+
  scale_fill_manual(values=c("#000000","#5798d1"))+
  scale_colour_manual(values=c("#000000","#5798d1"))

ggsave("compcdfplot.pdf", CompCumuDistplot, height=9, width=14.5, units="cm")
CompCumuDistplot
...
for(i in 1:length(p)){
  ...
  mixstat[j,i,6]<-NegBinomMixProbZero(p=p[i], R0D=R0D,k=kval, delta=delta[i]) #prob of no secondary infections
}  
  }

  #could include delta in the frame
#change 4 by pval by nstat array into  (4*pval) by nstat matrix/data frame
mstatdata <- data.frame(model="mixture", 
                        R0=rep(R0, length(p)), 
                        R0D=rep(R0D , length(p)),
                        p=p,
                        delta=delta, 
                        k=rep(k[1], length(p)), 
                        CaseVariance=mixstat[1,,1],
                        ExtinctionProbability=mixstat[1,,2],
                        R0star=mixstat[1,,3],
                        ChainVariance=mixstat[1,,4],
                        ProbabilityChainSizeLessEqual10=mixstat[1,,5], 
                        ProbChainSize1=mixstat[1,,6])
#stack each data frame corresponding to each k value below each other
for(i in 2:4) mstatdata<-bind_rows(mstatdata, data.frame(model="mixture", R0=rep(R0, length(p)), R0D=rep(R0D , length(p)),p=p, delta=delta,k=rep(k[i], length(p)), CaseVariance=mixstat[i,,1], ExtinctionProbability=mixstat[i,,2],R0star=mixstat[i,,3],ChainVariance=mixstat[i,,4],ProbabilityChainSizeLessEqual10=mixstat[i,,5],ProbChainSize1=mixstat[i,,6]))
...
nstat<-6
  ...
  basestat[j,6]<-NegBinomProbZero(R0=R0, k=kval) #Prob chain size of 1
  }

# each stat repeated times so for every value of p, there is a corresponding base statistic
bstatdata<-data.frame(model="standard", R0=rep(R0, length(p)), R0D=rep(R0D , length(p)),p=p,delta=delta, k=rep(k[1], length(p)), CaseVariance=rep(basestat[1,1],length(p)), ExtinctionProbability=rep(basestat[1,2],length(p)),R0star=rep(basestat[1,3],length(p)),ChainVariance=rep(basestat[1,4],length(p)), ProbabilityChainSizeLessEqual10=rep(basestat[1,5],length(p)), ProbChainSize1=rep(basestat[1,6],length(p)))
#put each data frame corresponding to each k value on top of each other
for(i in 2:4) bstatdata<-bind_rows(bstatdata, data.frame(model="standard", R0=rep(R0, length(p)), R0D=rep(R0D , length(p)),p=p, delta=delta,k=rep(k[i], length(p)), CaseVariance=rep(basestat[i,1],length(p)), ExtinctionProbability=rep(basestat[i,2],length(p)),R0star=rep(basestat[i,3],length(p)),ChainVariance=rep(basestat[i,4],length(p)), ProbabilityChainSizeLessEqual10=rep(basestat[i,5],length(p)), ProbChainSize1=rep(basestat[i,6],length(p))))
...
## Probability chain size of 1

```{r}
probzerosecondplot<-
ggplot(statdata)+
  geom_line(aes(x = p, y = ProbChainSize1, colour=Model), size=1)+
  facet_wrap(~k, labeller=label_bquote(k == .(k)))+
  ylab("Probability of zero secondary infections")+
  xlab("Proportion superspreaders p")+
  theme_minimal()+
  theme(panel.border = element_rect(color = "darkgrey", fill = NA, size = 1))+
  scale_colour_manual(values=c("#000000","#5798d1"))
ggsave("probzerosecondplot.pdf", probzerosecondplot, height=4, width=5)
probzerosecondplot
```
...
meanchainplot<-
ggplot(statdata)+
  geom_line(aes(x = p, y = MeanChain, colour=Model), size=1)+
  facet_wrap(~k, labeller=label_bquote(k == .(k)))+
  ylab("Mean chain size")+
  xlab("Proportion superspreaders p")+
  theme_minimal()+
  theme(panel.border = element_rect(color = "darkgrey", fill = NA, size = 1))+
  scale_colour_manual(values=c("#000000","#5798d1"))
ggsave("meanchainplot.pdf", meanchainplot, height=9, width=14.5, units="cm")
meanchainplot
```
\textbf{Figure X.}  Mean chain sizes are largest for large dispersion parameter $k$, small $p$ and large number of additional contacts. Mean chain sizes decrease as $p$ increases and approaches the value of the standard model as $p$ approaches 1. Mean chain sizes are larger in mixture models compared to the base model, even if $k>1$.
...
```{r}
stattab<-filter(statdata, p=="0.1")%>%select(Model, R0, R0D, p, delta, k, CaseVariance, ProbChainSize1, CVCases, MajorOutbreak, MeanChain, CVChain)
#write_csv(stattab, file="stats.csv")
```

Key statistics generated by mixture model with $k=4$ and mixture model with 20\% superspreaders:
```{r}
filter(statdata, p=="0.2" & k=="4" & Model=="mixture")
```
The following standard and mixture models have similar frequency of large clusters but the variance of cases in the mixture model is predicted to be higher than the standard model, leading to greater mean and CV of chain size conditioned on extinction (and I think standard model would have higher prob of zero cases)
```{r}
filter(statdata, p=="0.2" & k=="0.5" & Model=="standard")%>%select(Model, R0, R0D, p, delta, k, CaseVariance, CVCases, ProbChainSize1, MajorOutbreak, MeanChain, CVChain)
```
```{r}
filter(statdata, p=="0.1" & k=="2" & Model=="mixture")%>%select(Model, R0, R0D, p, delta, k, CaseVariance, CVCases, ProbChainSize1,MajorOutbreak, MeanChain, CVChain)
```
Key statistics generated by base model with $k=1$ and mixture model with 20\% superspreaders (note probability of major outbreak is similar to mixture model with $k=4$:
```{r}
filter(statdata, p=="0.2" & k=="1" & Model=="standard")
```
Percentage change in key statistics generated by mixture model with 10\% superspreaders relative to base model, $k=2$:
```{r}
df<-select(filter(statdata, p=="0.1" & k=="2"), ProbChainSize1, CaseVariance, ExtinctionProbability, ProbabilityChainSizeLessEqual10, CVCases, MajorOutbreak, MeanChain, CVChain)
...
```{r}
filmixstatsk05<-select(filter(statdata, Model=="mixture",k=="0.5"), ProbChainSize1,CaseVariance, ExtinctionProbability, ProbabilityChainSizeLessEqual10, CVCases, MajorOutbreak, MeanChain, CVChain)
stanstatsk05<-select(filter(statdata, Model=="standard",k=="0.5"), ProbChainSize1,CaseVariance, ExtinctionProbability, ProbabilityChainSizeLessEqual10, CVCases, MajorOutbreak, MeanChain, CVChain)
filmixstatsk1<-select(filter(statdata, Model=="mixture",k=="1"), ProbChainSize1,CaseVariance, ExtinctionProbability, ProbabilityChainSizeLessEqual10, CVCases, MajorOutbreak, MeanChain, CVChain)
stanstatsk1<-select(filter(statdata, Model=="standard",k=="1"), ProbChainSize1,CaseVariance, ExtinctionProbability, ProbabilityChainSizeLessEqual10, CVCases, MajorOutbreak, MeanChain, CVChain)
filmixstatsk2<-select(filter(statdata, Model=="mixture",k=="2"), ProbChainSize1,CaseVariance, ExtinctionProbability, ProbabilityChainSizeLessEqual10, CVCases, MajorOutbreak, MeanChain, CVChain)
stanstatsk2<-select(filter(statdata, Model=="standard",k=="2"), ProbChainSize1,CaseVariance, ExtinctionProbability, ProbabilityChainSizeLessEqual10, CVCases, MajorOutbreak, MeanChain, CVChain)
filmixstatsk4<-select(filter(statdata, Model=="mixture",k=="4"), ProbChainSize1,CaseVariance, ExtinctionProbability, ProbabilityChainSizeLessEqual10, CVCases, MajorOutbreak, MeanChain, CVChain)
stanstatsk4<-select(filter(statdata, Model=="standard",k=="4"), ProbChainSize1,CaseVariance, ExtinctionProbability, ProbabilityChainSizeLessEqual10, CVCases, MajorOutbreak, MeanChain, CVChain)

percentchangek05<-(filmixstatsk05-stanstatsk05)*100/stanstatsk05
percentchangek1<-(filmixstatsk1-stanstatsk1)*100/stanstatsk1
percentchangek2<-(filmixstatsk2-stanstatsk2)*100/stanstatsk2
percentchangek4<-(filmixstatsk4-stanstatsk4)*100/stanstatsk4

```

Mean minor outbreak size increases with $k$, whereas CV of cases and chains declines with increasing $k$
```{r}
filter(statdata, p=="0.01" & Model=="mixture")
```

```{r}
select(filter(statdata, p=="0.01" & Model=="standard"), k, MeanChain, CVChain)
```

```{r}
select(filter(statdata, p=="0.01" & Model=="mixture"), k, MeanChain, CVChain)
```
...
mjstart<-1-uniroot(NegBinomMixtureGen, c(0, 0.9999),  p=pval, R0D=R0Dval, k=kval, delta=(1-0)*deltaval)$root
controlstat<-mutate(controlstat, MajorOutbreakPercentageChange=(MajorOutbreak-mjstart)*100/mjstart)
```
...
```
```{r}
controlstat2 <- controlstat %>% 
  mutate(across('Model', 
                str_replace, 
                'Control on both groups', 
                'both groups')) %>%
  mutate(across('Model', 
                str_replace, 
                'Control on superspreading proportion', 
                'superspreading proportion')) %>%
  mutate(across('Model', 
                str_replace, 
                'Control on additional contacts', 
                'additional contacts')) %>%
  rename(ControlLevel = Control,
         Control = Model) %>% 
  mutate(Control = factor(Control, 
                        levels = c("both groups", 
                                   "superspreading proportion", 
                                   "additional contacts")))
```
...
extplot<-
ggplot(controlstat2)+
  geom_line(aes(x = ControlLevel, y = ExtinctionProbability, colour=Control, linetype = Control),
            size = 1, alpha=1,
            show.legend = FALSE)+
  ylab("Extinction Probability")+
  xlab("Control level c")+
  theme_minimal()+
  theme(panel.border = element_rect(color = "darkgrey", fill = NA, size = 1),
        plot.margin = margin(l = 24, unit = "pt"))+
  scale_colour_manual(values=c("#000000","#226e83","#e2908c"))  
```
...
varmeanplot<-
ggplot(controlstat2)+
  geom_line(aes(x = ControlLevel, y = VarMeanRatio, colour=Control, linetype = Control), 
            size = 1, alpha=1)+
  ylab("Variance to \nmean ratio")+
  xlab("Control level c")+
  theme_minimal()+
...
```{r}
# ggplot(controlstat)+
#   geom_point(aes(x = Control, y = MajorOutbreak, colour=Model))+
#   geom_line(aes(x = Control, y = MajorOutbreak, colour=Model))+
#   ylab("Probability of major outbreak ")+xlab("Control level c")
```
...
changeplot<-
  ggplot(controlstat2)+
  geom_line(aes(x = ControlLevel, y = MajorOutbreakPercentageChange, 
                colour=Control, linetype = Control), 
            size = 1, alpha=1,
            show.legend = FALSE)+
...
```{r Figure6}
library(egg)
controlplot <- ggarrange(varmeanplot +
...
ggsave("controlplot.pdf", controlplot, height=13, width=14.5, units="cm")
```
