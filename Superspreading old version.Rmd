---
title: "Superspreading branching process"
output: pdf_document
---
...
Poissonmixturebranch<- function(n,p, R0D, R0A) {  ## Poisson
	z <- c(1,rep(0,n))
	#one infected case in generation 0 #rmixture(z[i-1], probs = c(p, 1-p), models = expression(rpois(R0D), rpois(R0A)))
	for (i in 2:(n+1)) {
...
Poissonbranchw <- function(R0) {  ## Poisson
  #n = number of generations after generation 0
  n<-1
	z <- c(1,rep(0,n))
	#one infected case in generation 0
...
Poissonmixturebranchw<- function(p, R0D, R0A) {  ## Poisson
  #return the number of generations that it takes to have a cluster greater than 50
#  params<-as.list(c(R0D=R0D, R0A=R0A))
  n<-1 
	z <- c(1,rep(0,n))
	#one infected case in generation 0 #rmixture(z[i-1], probs = c(p, 1-p), models = expression(rpois(R0D), rpois(R0A)))
	#one infected case in generation 0
...
## Branching process
The process we consider is as follows:
\begin{itemize}
\item Null model: number of cases caused by a single infectious individual over the course of their infectious period: 
\begin{equation*}
    X \sim \text{ Poisson} R_0.
\end{equation*}
This is a Galton-Watson branching process since individuals infect a mean number $R_0$ of individuals at the end of their infectious period, and the infectious period (generation time) is constant.
\item Overdispersed/superspreading model: $$X \sim  p \, \text{Poisson} R_0^D + (1-p) \text{Poisson} R_0^A.$$ The number of cases $X$ generated by a single infectious individual over the course of their infectious period is a finite Poisson mixture with $0\leq p \leq 1$.
\item The model with superspreading assumes a fraction $p$ of cases are caused by direct contact with a known index case, and these cases are assumed to be Poisson distributed with mean $R_0^D$. 
\item A fraction $1-p$ of cases are caused by aerosol transmission, and these cases are Poisson distributed with mean $R_0^A$. These cases are due to an unknown index case (community transmission).
\item The expected number of cases of the Poisson mixture is $p R_0^D + (1-p) R_0^A$. We assume the expected number of cases is the same as the expected number of cases from the null model, $R_0$. If $p=0$, then $R_0 = R_0^A$ and if $p=1$ then $R_0 = R_0^D$.
\item We let $R_0^A = R_0^D + \delta$, where $\delta >0$. Biologically, it makes sense for $R_0^A$ to be greater than $R_0^D$ because on average, someone infected by an unknown source in the community is more likely to infect many people before realizing they are infected, whereas those infected by a known index case may be quicker to self-isolate (limit their contacts) once they become aware of their infection.
\item The expression for $R_0$ is
\begin{equation}
R_0 = R_0^D + (1-p)\delta,
\end{equation}
which lies between $R_0^D$ and $R_0^A$ if $0<p<1$.
\item We will compare the superspreading Poisson mixture model with a null Poisson model with mean $R_0$.
\end{itemize}
...
```{r}
data<-data.frame(cases=rmixture(1000, probs = c(4, 1), models = expression(rpois(1.5), rpois(16.5))))
ggplot(data, aes(x=cases)) + 
    geom_histogram()
data2<-data.frame(cases=rpois(1000, 4.5))
combdat<-bind_rows(list(Mixture=data,Baseline=data2),
                           .id="model")
ggplot(combdat,aes(cases,fill=model))+
   scale_fill_manual(values=c("red","blue"))+
  geom_bar(alpha=0.5) #geom_bar(mapping = aes(x = cut, y = ..prop.., group = 1), stat = "count") for proportions
```
...
\textbf{Figure 1.} For each value of $R_0$, the mixture models predict that a higher proportion of individuals remain uninfected than the baseline Poisson model, and the proportion decreases with increasing aerosol transmission.
...
\begin{itemize}
\item The probability generating function of the superspreading branching process is the weighted sum of Poisson generating functions (Johnson and Hoboken, 2005)
\begin{align}\label{eqn:Poissonmixgen}
G(z) = \sum_{k=0}^\infty P(X=k)z^k &= p \sum_{k=0}^\infty\frac{(R_0^D)^k}{k!} e^{-R_0^D}z^k+(1-p)\sum_{k=0}^\infty\frac{(R_0^A)^k}{k!}e^{-R_0^A}z^k \notag \\ 
&= p e^{R_0^D(z-1)}+ (1-p) e^{R_0^A(z-1)}
\end{align}
It is easy to check that this function satisfies all the requirements of a generating function. 
\item The probability of the branching process going extinct (probability of a minor outbreak) is given by numerically finding the root $z_\infty$ of the equation
$$ 
z = G(z) = p e^{R_0^D(z-1)}+ (1-p) e^{R_0^A(z-1)}.
$$
If $R_0<1$, then $z_\infty = 1$, i.e., the probability of a minor outbreak going extinct is certain. If $R_0 > 1$, then $0<z_\infty < 1$. The probability of a major outbreak is $1-z_\infty$. 
\end{itemize}
...
\textbf{Figure 2.} The probability of extinction for the baseline Poisson model declines as $R_0$ increases. The mixture models have much higher probability of extinction for the same $R_0$ (note here $R_0^D =1.5$.) The probability of extinction increases as the proportion of aerosol transmission decreases.
...
pval<-seq(0, 1, 0.01)
aerosol<-1-pval
R0p1<-R0(pval, R0D=1.5, R0adel(R0D=1.5, delta=1))
R0p2<-R0(pval, R0D=1.5, R0adel(R0D=1.5, delta=2))
R0p3<-R0(pval, R0D=1.5, R0adel(R0D=1.5, delta=4))
R0p4<-R0(pval, R0D=1.5, R0adel(R0D=1.5, delta=8))
...
#plot(rev((v4/R0p4))~(1-pval), type='l', col='purple', xlab="Fraction aerosol (1-p)", ylab="Variance to mean ratio")
#lines(rev((v3/R0p3))~(1-pval), type='l', col='cyan')
#lines(rev((v2/R0p2))~(1-pval), type='l', col='red')
#lines(rev((v1/R0p1))~(1-pval), type='l', col='blue')
...
```
\textbf{Figure 3.} The variance to mean ratio of the offspring distribution is a non-monotonic function of the proportion of aerosol transmission, and is at its highest for low to moderate levels of aerosol transmission. The Poisson mixture models are overdispersed for $0<p<1$.
...
```
\textbf{Figure 4.} The variance to mean ratio of the offspring distribution is a non-monotonic function of $R_0$, and is larger for larger values of $R_0^A$.
...
for (i in 1:length(R0Avals7)) {
  sse7[i]<-probSSE(0.7, R0D, R0Avals7[i], SSEpercentile(ptile=0.99, R0=R0(0.7, R0D, R0Avals7[i])))
  sse8[i]<-probSSE(0.8, R0D, R0Avals8[i], SSEpercentile(ptile=0.99, R0=R0(0.8, R0D, R0Avals8[i])))
  sse9[i]<-probSSE(0.9, R0D, R0Avals9[i], SSEpercentile(ptile=0.99, R0=R0(0.9, R0D, R0Avals9[i])))
}
...
for (i in 1:length(delta)) {
  sse7[i]<-probSSE(0.7, R0D, R0Avals[i], SSEpercentile(ptile=0.99, R0=R0(0.7, R0D, R0Avals[i])))
  sse8[i]<-probSSE(0.8, R0D, R0Avals[i], SSEpercentile(ptile=0.99, R0=R0(0.8, R0D, R0Avals[i])))
  sse9[i]<-probSSE(0.9, R0D, R0Avals[i], SSEpercentile(ptile=0.99, R0=R0(0.9, R0D, R0Avals[i])))
}
...
dev.off()
```
\textbf{Figure 6.} The probability of a superspreading event is an increasing function of aerosol reproduction number $R_0^A$ and saturates to $1-p$ for large $R_0^A$.

```{r}
data<-data.frame(cases=rmixture(1000, probs = c(4, 1), models = expression(rpois(1.5), rpois(16.5))))
p <- ggplot(data, aes(x=cases))  +
   geom_histogram()
p
```
...
\begin{itemize}
\item We define a chain that goes extinct at time $t$ by
$$
Y = \sum_{i=0}^{t-1}x_i
$$
with $x_i$ denoting the cumulative number of offspring in the $i^{th}$ generation, and $x_0 = 1$. 
\item The final size $Y$ upon extinction is a random variable with probability distribution $P(Y=y)$, $y=1,2,...$. We need to distinguish between when $R_0 < 1$ and $R_0 > 1$. 
\item From Yan (2008) we note that 
\begin{align*}
\sum_{y=1}^{\infty}P(Y=y) &= z_\infty,  \\  z_\infty &= \begin{cases}= 1 \text{ if } R_0 \leq 1 \\ < 1 \text{ if } R_0 >   1\end{cases}\end{align*}
\item Following Yan (2008) let $Q_Y(z) = \sum_{y=1}^{\infty}z^y P(Y=y)$. Finding the first and second derivatives of this function allows us to compute the mean and variance of the final size conditioned on extinction (chain size conditioned on a minor outbreak).
\item Using $Q_Y(z)=z G(Q_Y(z))$ (Mode and Sleeman 2000), the first and second derivatives of $Q_Y(z)$ are (Yan 2008)
\begin{align}\label{eqn:Qderiv}
Q_Y'(z) &= G(Q_Y(z))+ z G'(Q_Y(z))Q_Y'(z),\\
Q_Y''(z) &= 2 G'(Q_Y(z))Q_Y'(z)+ z (G''(Q_Y(z))(Q_Y'(z))^2 + G'(Q_Y(z))Q_Y''(z)).
\end{align}
\item If $R_0 < 1$, then 
$$
E(Y) = Q_Y'(1) = \frac{1}{1-R_0},
$$
which is valid for any distribution of $X$.
\item To compute the variance assuming $R_0 <1$, we use $V(Y) = Q_Y''(1)+ Q_Y'(1)- (Q_Y'(1))^2$,
$$
V(Y) = \frac{V(X)}{(1-R_0)^3}
$$
\item If $R_0 > 1$, then $0 < z_\infty < 1$ and we compute $R_0^* = G'(z_\infty) < 1$, i.e., the basic reproduction number conditioned on extinction of the chain. Then we can calculate the mean and variance of the chain size conditioned on a minor outbreak occurring (Yan 2008),
\begin{align*}
E(Y|\text{minor outbreak}) &= \frac{1}{1-R_0^*} \\
V(Y|\text{minor outbreak}) &= \frac{z_\infty Q_Y''(z_\infty)+R_0^{*}(1-R_0^*)}{(1-R_0^*)^3}
\end{align*}
\item As $R_0$ increases, the probability of a minor outbreak $z_\infty$ declines. Therefore $R_0^*$ declines with $R_0$, and so the mean chain size will also decline (Figure 8.) For example, for the baseline Poisson model, $R_0^*= G'(z_\infty) = R_0 e^{(R_0(z_\infty -1 ))}$, which decreases as $z_\infty$ decreases.
\item There are also results for final size conditioned on a major outbreak occurring (Yan 2008), not included here.
\end{itemize}
...
\textbf{Figure 7.} The mean chain size decreases with increasing proportion of aerosol transmission. 
...
dev.off()
```
\textbf{Figure 8.} The mean chain size decreases with increasing $R_0$. This is because the probability of a major epidemic increases with $R_0$ (less likely to have a transmission chain that does not result in a major epidemic).
...
```
\textbf{Figure 9.} The probability of a major epidemic increases with $R_0$ (less likely to have a transmission chain that does not result in a major epidemic). T
...
```
\textbf{Figure 10.} The probability of a major epidemic increases with $R_0^A$ (less likely to have a transmission chain that does not result in a major epidemic).
...
```
\textbf{Figure 11.} The variance to mean ratio of the chain size declines as a function of $R_0$. The level of heterogeneity (overdispersion) decreases as $R_0$ increases.
...
```
\textbf{Figure 12.} The variance to mean ratio of the chain size declines as a function of the level of aerosol transmission because the probability of a major epidemic increases as a function of the level of aerosol transmission.
...
\textbf{Figure 13.} The probability of a major epidemic increases as a function of the level of aerosol transmission.
...
```{r}
#simulated cluster sizes for the Poisson distribution:
...
\item The chain size distribution can be used to find the probability of the size of a chain exceeding the $n$th percentile, a measure of superspreading (Lloyd Smith et al. 2005),
\begin{equation}
    \psi_n = P(Y>y_n)=  1 - P(Y \leq y_n).
\end{equation}
...
```{r}
extprob<-uniroot(PoissonGen, c(0, 0.99), R0=R0(0.8, 1.5, 16.5))$root
...
```
\textbf{Figure 14.} Chain size distributions conditional on extinction for a Poisson mixture are more fat-tailed  than the corresponding Poisson distribution with the same mean ($R_0 = 4.5$). Here $R_0^D = 1.5$. 
...
```{r}
extprob1<-uniroot(PoissonGen, c(0, 0.99), R0=R0(0.8, 1.5, 5.5))$root
extprob2<-uniroot(PoissonGen, c(0, 0.99), R0=R0(0.8, 1.5, 9.5))$root
...
```
\textbf{Figure 15.} Chain size distributions conditional on extinction for a Poisson mixture vary little with increasing $\delta$. Here $R_0^D = 1.5$.  
...
```{r}
extprob1<-uniroot(PoissonGen, c(0, 0.99), R0=R0(0.95, 1.5, 9.5))$root
extprob2<-uniroot(PoissonGen, c(0, 0.99), R0=R0(0.75, 1.5, 9.5))$root
...
```
\textbf{Figure 16.} The burstiness of the Poisson mixture offspring distributions do not manifest in the corresponding chain size distributions conditioned on extinction. The 5 percent aerosol model has a fatter tail than the 25 percent aerosol model, but $R_0$ is smaller. As $R_0$ decreases, minor outbreaks occur with greater probability in the mixture models. The Poisson model with $R_0 = 1.9$ looks fairly similar to the mixture model with $R_0 = 1.9$. As $R_0$ increases the difference between the mixture chain size distribution and the Poisson distribution becomes larger. Note that the distribution is conditional on extinction, i.e., we have divided it by the probability of extinction. The analytical expression for the chain size distribution will not capture burstiness. Major outbreaks occur with probability $1-z_\infty$. To compare the analytical chain size distribution with a distribution obtained via simulation, we must not divide it by $z_\infty$.
## Previous assumptions
\begin{itemize}
\item We assume that $R_0^D < R_0^A$. If $0 < p < 1$ then $R_0$ has to lie between $R_0^D$ and $R_0^A$. Assuming $R_0 = p R_0^D + (1-p) R_0^A,$ $R_0^D < R_0^A$ and $0 < p < 1$, we can prove this statement by contradiction. Suppose that $R_0<R_0^D$. Then by definition we have $p R_0^D + (1-p) R_0^A<R_0^D$ or equivalently $(1-p) R_0^A<(1-p)R_0^D.$ Since $p \neq 1$ dividing both sides by $1-p$ yields $R_0^A<R_0^D,$ which contradicts the assumption that $R_0^D < R_0^A$. On the other hand, suppose  $R_0>R_0^A$. By definition $p R_0^D + (1-p) R_0^A>R_0^A$ or equivalently $p R_0^D >R_0^A- (1-p) R_0^A$, yielding $p R_0^D >p R_0^A$. Since $p\neq 0$ dividing both sides yields  $R_0^D> R_0^A,$ which contradicts the assumption that $R_0^D < R_0^A$. Therefore if $0<p<1$, $R_0^D < R_0 < R_0^A$.
\item Assuming $R_0$ and $R_0^D$ are known and $R_0<R_0^D$, we find $R_0^A$ as a function of $p$ using the expected number of cases of the Poisson mixture:
\begin{equation}\label{eqn:R0A}
R_0^A = \frac{R_0 - p R_0^D}{1-p},\quad  0\leq p <1
\end{equation}
We require $R_0^A > 0$. This will hold provided $p<R_0/R_0^D$. Since $R_0^D<R_0$ then $R_0^A$ is always positive. Note $R_0^A$ is bounded below at $R_0$ when $p=0$, but $R_0^A$ has no upper bound ($\lim_{p\rightarrow 1} R_0^A = +\infty$.)
\item Increasing the proportion of cases caused by direct contact $p$ will increase $R_0^A$. 
\item Assuming $R_0$ and $R_0^D$ are known, we find $R_0^A$ as a function of $p$ using the expected number of cases of the Poisson mixture:
\begin{equation*}
R_0^A = \frac{R_0 - p R_0^D}{1-p},\quad  0\leq p <1
\end{equation*}
We require $R_0^A > 0$. This will hold provided $p<R_0/R_0^D$. Since $p<1$ by definition of $R_0^A$, then $1<R_0/R_0^D<$ and so we require $R_0^D < R_0$. Note $R_0^A$ is bounded below at $R_0$ when $p=0$, but $R_0^A$ has no upper bound ($\lim_{p\rightarrow 1} R_0^A = +\infty$.)
\item If we assume $R_0^A>R_0^D$, then $R_0 - p R_0^D/(1-p) > R_0^D$ by definition, or equivalently, $R_0 > R_0^D$. Biologically, it makes sense for $R_0^A$ to be greater than $R_0^D$ because on average, someone infected by an unknown source is more likely to infect many people before realizing they are infected, whereas those infected by a known index case may self-isolate (limit their contacts) once they become aware that they infected.
\item The mean of the branching process (expected number of cases) is $E(X) = R_0 = p R_0^D + (1-p) R_0^A$. Some algebra yields the variance of the branching process,
$$
V(X) = R_0 + \frac{p}{1-p}(R_0-R_0^D)^2, \quad 0 < p <1
$$
Note that $V(X)$ is always greater than the expected number of cases, $R_0$, and is unbounded in the limit as $p\rightarrow 1$. So if most cases are due to direct contact, and only a small proportion $1-p$ are due to aerosol transmission, the variance to mean ratio will be much greater than one.
\item Variance as a function of $R_0^A$ has a stationary point at $R_0^A = R_0^D - 1/2p$. But we assume $R_0^D<R_0^A$, so the stationary point is outside of the interval $(R_0^D, R_0^A)$, and since variance is concave up, the variance is an increasing function of $R_0^A$ in the interval $(R_0^D, \infty)$. If we assume that the maximum value $R_0$ can have is one, the maximum value (upper bound) that $R_0^A$ can have for a given value of $R_0^D$ is $R_0^A = (1-p R_0^D)/(1-p).$
\item Variance as a function of $R_0^D$ (assuming $R_0^A$ is fixed) has a stationary point at 
\begin{equation}\label{eqn:RodS}
 R_0^D = R_0^A - p/2p(1-p),   
\end{equation}
which is strictly less than $R_0^A$, so it is possible for variance to be a decreasing function of $R_0^D$ somewhere in the interval $(0, (1-(1-p) R_0^A)/p)$ where $(1-(1-p) R_0^A)/p$ is the maximum value that $R_0^D$ can have for a given value of $R_0^A.$
\item By L'Hopitals rule the upper bound for the location of the stationary point (i.e. limit as $p$ approaches zero from the right) is $R_0^A - 1/2$, and the location of the stationary point decreases as $p$ increases. If $p > 1-1/2R_0^A$, the stationary point (eqn. \ref{eqn:RodS}) is negative and since variance is concave up, it will be an increasing function of $R_0^D$. 
\item To allow for comparison with a null Poisson model with mean $R_0$, we assume that the mean of the Poisson mixture model is fixed at $R_0$, and the variance can be rewritten as
\begin{align}\label{eqn:varfixed}
     V(X) &=  R_0(1-R_0) + p (R_0^D)^2 +(1-p)(R_0^A)^2 \notag \\ &=R_0(1-R_0) +(R_0^A)^2- p \left (R_0^A)^2 -(R_0^D)^2 \right )
\end{align}
\item To compare the Poisson mixture offspring distribution with the null Poisson model, we will assume $R_0$ remains constant (the common parameter of both models) but the contribution of direct contact $p$ and the aerosol reproduction number $R_0^A$ can increase. To keep $R_0 = pR_0^D+(1-p)R_0^A $ fixed at a desired value, as $p$ approaches 1, the reproduction number for aerosol exposure events $R_0^A$ must get increasingly large to compensate for the increasing contribution to $R_0$ of direct exposure events with mean $R_0^D$ (the proportion $pR_0^D$). For example, if $R_0 =3$, and $R_0^D=0.5$, then $R_0^A$ must equal 13 to have 80\% of cases due to direct contact and 20\% of cases due to aerosol transmission, i.e., $3 = (0.8)( 0.5) + (0.2)( 13) $. Increasing $p$ to 0.9 but keeping $R_0$ constant at 3 and $R_0^D=0.5$, then $R_0^A$ must be equal to 25.5, larger than the required $R_0^A$ at $p=0.8$. Recall that $R_0^A = (R_0 - p R_0^D)/(1-p).$
\item Since $R_0^A$ must increase as $p$ increases, then the variance \eqref{eqn:varfixed} must increase as $R_0^A$ increases. The derivative of \eqref{eqn:varfixed} is an increasing function of $R_0^A.$
\item Equation \eqref{eqn:var} is a concave up quadratic function of both $R_0^A$ and $R_0^D$ since its second derivative $\frac{dV}{dR_0^D}=\frac{dV}{dR_0^A}=2p(1-p)$ is always positive if $0<p<1$.
...
#generate the cluster distriubiton
p2<-0.8
R0D2<-1.5
R0A2<-4
...
ggplot(clusterP2) +
   geom_bar(mapping = aes(x = clusters, y = stat(prop))) #, group = 1
     #   geom_histogram(binwidth=1, aes(y = ..density..))+
  #geom_density(col = "red")
  #geom_line(mapping=aes(x=clusters, y=age2))

clusterPtab<-as.data.frame(table(clusterP2))
num<-dim(clusterP2)[1]
clusterPtab2<-mutate(clusterPtab, prop=Freq/num)
data<-select(clusterPtab2, clusterP2, prop)
ggplot(data, aes(y=prop)) + 
    geom_bar()
```
