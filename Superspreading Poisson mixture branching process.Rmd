...
clusterM20<-clusterM%>%filter(clusters<21)
dim(clusterM20) #use to obtain the proportion that have gone extinct
...
clusterthM20<-data.frame(n=1:20, p=csize2/extprobM) 


plot<-ggplot() + 
  geom_bar(data = clusterM20, aes(x = clusters, y = stat(prop))) + 
  geom_point(data = clusterthM20, aes(x=n, y=p))+
  geom_line(data = clusterthM20, aes(x=n, y=p))
plot
...
```{r}
colSums(mutate(clusterthM20, n*p))
```

The theoretical mean $1/(1-R_0^*)$ is
```{r}

theor.mean<-mean.chain(PoissonMixR0star(p=0.8, R0D=1.5, R0A=4))
theor.mean
```


Up to cluster sizes of 10:
...
```
The mean of the clusters obtained from simulation of the branching process is an overestimate:
```{r}
mean(clusterM2$clusters)
```
...
for(n in 3:nend){
nvec<-1:(n-1)
gvec<-gN(nvec, p2, R0D2, R0A2) #need n-1 g derivatives

#The n-1th derivative of (a+b)^n evaluated at s = 0:
#the for loop should work for chain sizes greater than 2
x1<-fNr(n, 1, p2, R0D2, R0A2)*BellB(n-1,1,gvec)
for(k in 2:(n-1)) x1<-x1+fNr(n, k, p2, R0D2, R0A2)*BellB(n-1,k,head(gvec,-(k-1)))

#vector of P(chain size = n)
csize32[n-2]<-x1/factorial(n)
rm(x1)
}
csize12<-p2*exp(-R0D2)+(1-p2)*exp(-R0A2)
csize22<-fNr(2, 1, p2, R0D2, R0A2)*gN(1,p2,R0D2, R0A2)/2
#compute chain size distribution
csize2<-c(csize12, csize22, csize32)
#Compare with Borel distribution
```
